...
type UpdateMedia = z.infer<typeof updateMedia>;

@Controller('/media')
export class UpdateMediaController {
  constructor(private readonly updateMediaUseCase: UpdateMediaUseCase) { }
  ...
  @HttpCode(200)
  async handle(
    @Body(new ZodValidationPipe(updateMedia)) updateMedia: UpdateMedia,
    @Param('id') id: string
  ) {
    const result = await this.updateMediaUseCase.execute({
      ...updateMedia,
      id
    });
    return result;
